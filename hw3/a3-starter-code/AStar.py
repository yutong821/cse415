...
''' starter-file-for.py  CHANGE THIS WHEN YOU EDIT IT, TO AStar.py
by  (Yutong Liu)
...
# STEP 1a. Compute f(s0) = g(s0) + h(s0) and put [s0, f(s0)] on a list OPEN
...
# STEP 4. Generate the list L of [s',f(s')] pairs where the s' are the successors of s and
#         their f values are computed using f(s') = g(s')+h(s'). Consider each [s',f(s')].
   ...
   # EACH SUCCESSOR.***
    for op in Problem.OPERATORS:
      if op.precond(S):
        ...
        new_g = g[S] + S.edge_distance(new_state)
